...
let max_time;               // maximum time between screams, must be greater than or equal to min_time
let min_time;               // minimum time between screams, must be greater than config.minimal_time
let start_silence = false;  // boolean, true if server should start with silence, false if scream
...
function join(msg, mem) {
    if(toggled && mem.voice.channel) {
        ...
        channel.join().then(conn => {
            ...
            // ytdl('https://www.youtube.com/watch?v=XdQPPh5eA_8')
            /* Supplied by some nice friends:
                start0 - Jack
                start1 - Cam
            */
...
function deleteFile(id) {
    // delete last recording and max recording
    ...
    if(msg.content.startsWith(config.prefix)) {
        ...
        switch(command) {
            ...
            case 'join':
                ...
                break;
            case 'delete':
                deleteFile(msg.member.user.id);
                break;
            case 'set': // TODO: change server settings
                break;
        }
    }
});
